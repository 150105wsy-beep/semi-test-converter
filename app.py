...
with st.sidebar:
    ...
    with st.expander("💡 点击查看文件名规范与示例", expanded=False):
        ...
        **文件名参数自动解析规则：**
        ...
        - **L 参数**：提取 `L` 前的数字
          - 示例：`Ids_vgs_0.5L_vds0.05.csv` → L=0.5
          - 示例：`test_data_1.2L_vds1.8.csv` → L=1.2
        
        - **Vds 参数**：提取 `vds` 后的数字（不区分大小写）
          - 示例：`Ids_vgs_0.5L_vds0.05.csv` → Vds=0.05
          - 示例：`test_data_1.2L_vds1.8.csv` → Vds=1.8
    ...
    # 参数输入
    st.subheader("参数设置")
    vds_manual = st.text_input("Vds (变量 a)", value="0.1", help="浮点数或字符串，默认值：0.1")
    ...
    # Curve 替换顺序控制
    reverse_curve_order = st.checkbox(
        "反转 Curve 替换顺序",
        value=False,
        help="勾选后，curve 值的替换顺序将反转（从 0 到 -1.8）"
    )
    
    # 显示当前替换顺序
    curve_values_forward = ["-1.8", "-1.35", "-0.9", "-0.45", "0"]
    curve_values_reverse = ["0", "-0.45", "-0.9", "-1.35", "-1.8"]
    current_curve_values = curve_values_reverse if reverse_curve_order else curve_values_forward
    
    st.caption("**当前替换顺序：**")
    order_list = "\n".join([f"{i+1}. curve {{ {val} }}" for i, val in enumerate(current_curve_values)])
    st.text(order_list)
    
    st.markdown("---")
    st.caption("💡 提示：上传文件后，如您发现数据对应Vbs搞反了，点击顺序转换按钮重新生成")
...
def parse_parameters_from_filename(filename):
    ...
    Returns:
        tuple: (l_value, vds_value, success)
            - l_value: 解析到的 L 值（字符串），失败返回 None
            - vds_value: 解析到的 Vds 值（字符串），失败返回 None
    ...
    # 解析 Vds 参数：提取 vds 后的数字（如 vds1.8 -> 1.8）
    # 支持格式：vds数字、vds_数字、vds-数字 等
    vds_pattern = r'vds[_\s-]*([\d.]+)'
    vds_match = re.search(vds_pattern, filename_lower)
    if vds_match:
        vds_value = vds_match.group(1)
    
    success = l_value is not None or vds_value is not None
    return l_value, vds_value, success
...
def convert_csv_to_txt(csv_content, vds, w, l, reverse_order=False):
    """
    ...
    Args:
        ...
        w: W 参数值
        l: L 参数值
        reverse_order: 是否反转 curve 替换顺序，默认为 False
    ...
    output_lines.append(f"Page (name=Ids_Vgs_Vbs,x=Vgs,p=Vbs,y=Ids){{Vds={vds},W={w},L={l},T=25}}")
    
    # B. 序列状态机替换
    curve_values_forward = ["-1.8", "-1.35", "-0.9", "-0.45", "0"]
    curve_values_reverse = ["0", "-0.45", "-0.9", "-1.35", "-1.8"]
    curve_values = curve_values_reverse if reverse_order else curve_values_forward
    curve_index = 0
...
def main():
    ...
    for uploaded_file in uploaded_files:
        ...
        if force_manual:
            # 强制手动模式：使用手动输入的值
            l_value = l_manual
            vds_value = vds_manual
            w_value = w_manual
            status = "🔵 手动强制覆盖"
            status_color = "blue"
            source_info = f"L={l_value}, Vds={vds_value}"
        else:
            # 自动解析模式：尝试从文件名解析
            l_parsed, vds_parsed, parse_success = parse_parameters_from_filename(filename)
            
            # 确定最终使用的值（解析失败则使用手动默认值）
            l_value = l_parsed if l_parsed is not None else l_manual
            vds_value = vds_parsed if vds_parsed is not None else vds_manual
            w_value = w_manual  # W 固定为 1
            
            if parse_success:
                if l_parsed is not None and vds_parsed is not None:
                    status = f"🟢 自动解析成功 (L={l_parsed}, Vds={vds_parsed})"
                    status_color = "green"
                    source_info = f"L={l_parsed}, Vds={vds_parsed}"
                elif l_parsed is not None:
                    status = f"🟢 自动解析成功 (L={l_parsed})，Vds 使用默认值"
                    status_color = "green"
                    source_info = f"L={l_parsed}, Vds={vds_value} (默认)"
                else:
                    status = f"🟢 自动解析成功 (Vds={vds_parsed})，L 使用默认值"
                    status_color = "green"
                    source_info = f"L={l_value} (默认), Vds={vds_parsed}"
            else:
    ...
    if file_params:
        ...
        df_data = {
            ...
            'Vds': [fp['vds'] for fp in file_params],
